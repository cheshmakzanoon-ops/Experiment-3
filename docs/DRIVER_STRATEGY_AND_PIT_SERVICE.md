...
The HUD and cockpit display show a delta only after a complete valid reference exists. The physical wheel also displays brake bias, power differential setting, deployment mode and battery state; individual shift LEDs follow measured engine RPM. Protocol version 5 adds the service clock, actual delta/reference availability and driver-decision diagnostics. The fixed snapshot stride is 224 floats per car and the telemetry schema has 197 uniquely named channels. Recording and replay use the same state.
