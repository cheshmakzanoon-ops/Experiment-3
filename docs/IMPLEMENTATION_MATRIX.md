...
| 103 | UNIT TESTS | 497 unit/property/regression tests in 51 files, including 37 new road-contact, skid, recording and pit-priority regressions. All previous coverage remains. Two new production regressions fail against the unchanged baseline Vehicle and pass after the repair. Full-project correctness is not inferred from test count. |
...
| 117 | PLAYWRIGHT TESTS | Complete suite retained and expanded to 26 cases, including actual wet-session replay/modal/seek/audio/focus ownership and production floor-contact spark pixels. Build, native scenarios and all three browser shards are required before playable publication. Local WebGL is unavailable; use the complete GitHub run for the exact published revision, not only the two selected candidate cases. |
