...
| 117 | PLAYWRIGHT TESTS | Complete suite retained and expanded to 25 cases, including actual wet-session replay/modal/seek/audio/focus ownership. Build, native scenarios and all three browser shards are required before playable publication. Local full-application navigation is blocked by browser policy; read the CI run for the exact published revision rather than inferring a pass from a prior run. |
...
| 139 | VERSIONED SAVES | Settings version 6 migrates versions 1–5, preserving independent version-4 graphics and calibration. Controller indices/actions/conflicts are validated. Recorded protocol is version 8; settings and replay version domains are separate. |
