...
| 37 | TRACK SURFACE STATE GRID | 512 × 7 physics water/rubber/marble/temperature cells. Version-7 worker and three-channel replay surface records preserve original marble state; existing GPU texture B carries local density. No off-track or pit contact reads/depletes neighboring asphalt rubber/marbles. |
...
| 80 | TELEMETRY DATA | 203 actual-state channels at 60 Hz; the first 199 column names/positions are frozen under a hash regression. Four cumulative wheel-pickup columns are appended. Bounded transport/ring and separate CSV-worker export. |
...
| 82 | REPLAY SYSTEM | Original 15 Hz numeric replay and surface history retained. Velocity-informed wrapped wheel phase and physical suspension travel interpolate in all LODs; camera/effect clocks follow presented time. No replay/CSV samples are rewritten. |
...
| 103 | UNIT TESTS | 427 unit/property/regression tests in this reconciled local continuation, including 45 controller-action cases and 9 wheel-pose cases. The concurrent remote camera regression suite is retained alongside the stronger slow-frame/subdivision tests. Full local check result is evidence for its exact source, not universal correctness. |
...
| 115 | AI TESTS | Simulation/core fingerprint 30c155c669e4…: new ten-car dry 50-lap and 100-lap results both pass with zero impacts, zero off-track time and minimum component health 1. Exact measured source identities are reported in CONSTRUCTION_AND_ENDURANCE.md; do not substitute a historical fingerprint. |
...
| 117 | PLAYWRIGHT TESTS | Existing complete Playwright suite retained. Four real local Chromium component tests cover camera, road/tread/particles, engine PCM and instrument/wheel transforms. Full application and two-origin navigation were blocked locally; no full-suite pass is claimed. |
...
| 139 | VERSIONED SAVES | Settings version 6 migrates versions 1–5, preserving independent version-4 graphics and calibration. Controller indices/actions/conflicts are validated. Recorded protocol stays version 7; settings and replay version domains are separate. |
