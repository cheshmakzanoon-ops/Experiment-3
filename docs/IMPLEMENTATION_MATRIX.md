...
Current cockpit continuation: **527 unit cases / 54 files; 29 browser cases**. See [scope, measured costs and open gates](COCKPIT_REFERENCE_CONTINUATION.md).
...
| 103 | UNIT TESTS | 527 unit/property/regression tests in 54 files. Includes 37 earlier contact/skid/recording/pit regressions, 15 renderer/HDR regressions and 15 new cockpit/material/reflection-time/batching cases. Earlier coverage is retained. Full-project correctness is not inferred from test count. |
...
| 117 | PLAYWRIGHT TESTS | Complete suite expanded to 29 cases, including high-preset clear/rain HDR captures and real-input steering, selectors, pause, rewind and resource disposal. Local Chromium/WebGL2 now runs under Xvfb/software rendering. Build, native scenarios and all three browser shards remain required for playable publication; selected tests do not replace full CI. |
...
| 122 | VISUAL QUALITY PASSES | Reference-led bodywork/material/daylight pass followed by fitted cockpit geometry, cloth detail and recorded-state selectors. Dry/wet and articulated production renders are inspected. The result remains substantially below F1 25 photorealism; see both reference reviews. |
