...
interface Sample { id: number; frame: Float32Array; history: Float32Array[]; water: Float32Array; rubber: Float32Array; marbles: Float32Array }
interface VenueOptions { weather: 'clear' | 'rain'; night: boolean }
...
class VenueSurvey {
  ...
  constructor(readonly options: VenueOptions) {
    this.simulation = new Simulation({ ...DEFAULT_OPTIONS, mode: 'practice', opponents: 0,
      weather: options.weather, compound: options.weather === 'rain' ? 'wet' : 'medium', seed: 1887 });
    this.simulation.autoPlayer = true;
    const recent: Float32Array[] = [], chosen = new Map<number, Sample>();
    const length = this.simulation.track.length, spacing = length / 20, base = carBase(0);
    for (let i = 0; i < 120 * 180 && chosen.size < 20; i++) {
      this.simulation.step(1 / 120);
      if (i % 4 !== 0) continue;
      const frame = this.simulation.makeFrame(); recent.push(frame);
      if (recent.length > 46) recent.shift();
      const s = frame[base + F.S], id = Math.round(s / spacing) % 20;
      ...
      if (recent.length >= 40 && frame[base + F.SPEED] > 5 && distance < 3 && !chosen.has(id))
        chosen.set(id, { id, frame, history: recent.slice(),
          water: this.simulation.track.water.slice(), rubber: this.simulation.track.rubber.slice(),
          marbles: this.simulation.track.marbles.slice() });
    }
    this.samples = [...chosen.values()].sort((a, b) => a.id - b.id);
    if (this.samples.length !== 20) throw new Error(`Incomplete real-lap survey: ${this.samples.length}/20 cameras`);
    ...
    this.renderer.setPixelRatio(1); this.renderer.setSize(1280, 720);
    this.renderer.toneMapping = T.ACESFilmicToneMapping;
    this.renderer.shadowMap.enabled = true; this.renderer.shadowMap.type = T.PCFSoftShadowMap;
    this.sun.castShadow = true; this.sun.shadow.mapSize.set(2048, 2048);
    Object.assign(this.sun.shadow.camera, { left: -65, right: 65, top: 65, bottom: -65, near: 1, far: 600 });
    this.sun.shadow.bias = -0.0001; this.sun.shadow.normalBias = 0.035;
    configureSky(this.sky); this.sky.scale.setScalar(10000);
    ...
    this.director = new TracksideDirector(this.simulation.track);
    this.scene.add(this.circuit.group, this.car.root, this.sky, this.sun, this.sun.target, this.fill,
      this.venue.root, this.effects.group);
    this.scene.fog = new T.FogExp2(0x8aa6c2, 0.00025);
  }
  capture(index: number) {
    if (!Number.isInteger(index) || index < 0 || index >= 27) throw new Error('Invalid venue survey view');
    const sample = this.samples[index < 20 ? index : 4];
    const frame = sample.frame, before = frame.slice(), base = carBase(0);
    ...
    this.sky.visible = !this.options.night;
    ...
    this.camera.position.copy(this.director.position); this.camera.lookAt(this.director.gaze);
    ...
    if (index >= 20 && index < 26) {
      ...
      this.camera.position.copy(anchor).add(new T.Vector3(10, 8, 16).applyAxisAngle(new T.Vector3(0, 1, 0), site.yaw));
      this.camera.lookAt(anchor); this.camera.fov = 50;
    } else if (index === 26) {
      name = 'occupied-grandstand'; anchor.copy(this.circuit.at(780, 22, 4));
    ...
    this.environment.update(this.renderer, this.scene, light.cover);
    this.scene.updateMatrixWorld(true);
    const rendered = () => {
      this.renderer.info.reset(); this.renderer.render(this.scene, this.camera);
      // Readback forces completion; duration is software-render diagnostic only.
      return this.canvas.toDataURL('image/png');
    };
    const start = performance.now(), image = rendered(), elapsedMs = performance.now() - start;
    const memory = { ...this.renderer.info.memory };
    const waterUnchanged = this.savedTrack.every((v, i) => v === this.simulation.track.water[i]);
    return { view: name, image, elapsedMs, memory, draws: this.renderer.info.render.calls,
      triangles: this.renderer.info.render.triangles, tick: frame[H.TICK], time: frame[H.TIME],
  ...
  dispose() {
    disposePhase27Scene(this.scene); this.environment.dispose(); this.renderer.dispose(); this.canvas.remove();
  }
}
...
export function beginPhase27Venue(options: VenueOptions) {
  ...
  return { views: 27, cameras: current.samples.map((s) => s.id), serviceAreas: current.circuit.serviceSites.length,
    realSimulationTicks: current.simulation.tick, crowdClusters: current.circuit.crowdClusters.length };
}
...
export function endPhase27Venue() { current?.dispose(); current = null; }
