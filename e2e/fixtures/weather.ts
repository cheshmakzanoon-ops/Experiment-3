...
import { circuitLightState, lightingDirection } from '../../src/rendering/daylight.ts';
...
export function verifyWeatherGPU() {
  ...
  const light = circuitLightState(frame[H.CLOUD], frame[H.RAIN], 'day');
  const sun = new T.DirectionalLight(0xffffff, light.sun);
  sun.position.copy(lightingDirection('day'));
  const fill = new T.HemisphereLight(0xffffff, 0xffffff, light.fill);
  applyCircuitLightPalette(sun, fill, frame[H.CLOUD], 'day');
  scene.add(sun, fill);
  const effects = new Effects();
  scene.add(effects.group);
  const blank = new Uint8Array(128 * 128 * 4),
    active = new Uint8Array(blank.length);
  try {
    ...
    renderer.render(scene, camera);
    renderer.readRenderTargetPixels(target, 0, 0, 128, 128, active);
    let changedPixels = 0;
    for (let i = 0; i < active.length; i += 4)
      if (
        active[i] !== blank[i] ||
        active[i + 1] !== blank[i + 1] ||
        active[i + 2] !== blank[i + 2]
      )
        changedPixels++;
    const diagnostics = effects.diagnostics();
    effects.clear();
    effects.update(frame, 1 / 60, false);
    renderer.render(scene, camera);
    renderer.readRenderTargetPixels(target, 0, 0, 128, 128, active);
    ...
    return {
      ...
      clearDifferences,
      diagnostics,
      rainKind: PARTICLE_KIND.RAIN,
      simulationWind: [frame[H.WIND_X], frame[H.WIND_Z]],
      speedMps: frame[o + F.SPEED],
    };
  } finally {
