...
export function addWheelMechanicalDetail(
  ...
  const nut = mesh(lock, new T.CylinderGeometry(0.029, 0.031, 0.02, 6), m.metal,
    side * (halfWidth + 0.029), 0, 0);
  nut.rotation.z = Math.PI / 2;
  const bore = mesh(lock, new T.CircleGeometry(0.013, 16), m.dark,
    side * (halfWidth + 0.040), 0, 0);
...
export function addTailMechanicalDetail(parent: T.Group, m: MechanicalMaterials) {
  ...
  mesh(tail, sculptedLoft([
    [-2.27, -0.205, 0.047, 0.039],
    [-2.10, -0.195, 0.074, 0.052],
    [-1.83, -0.18, 0.095, 0.065],
  ], 0.6, 0.6), m.carbon);
  const outlet = mesh(tail, new T.CylinderGeometry(0.038, 0.035, 0.15, 24, 1, true), m.metal,
    0, -0.015, -2.117);
  outlet.rotation.x = Math.PI / 2;
  mesh(tail, new T.TorusGeometry(0.037, 0.003, 6, 24), m.metal, 0, -0.015, -2.193);
  const interior = mesh(tail, new T.CircleGeometry(0.034, 24), m.dark, 0, -0.015, -2.10);
...
export function buildHelmet(parent: T.Group, m: MechanicalMaterials) {
  ...
  parent.position.set(0, 0.17, -0.40);
  const shellGeometry = new T.SphereGeometry(0.137, 40, 28);
  ...
  for (let i = 0; i < shellPositions.count; i++) {
    ...
    const chin = T.MathUtils.clamp((-y - 0.032) / 0.095, 0, 1);
    shellPositions.setXYZ(i, x * (1 - 0.06 * chin), y,
      z + Math.max(0, z / 0.137) * 0.026 * Math.sin(Math.PI * chin));
  }
  shellGeometry.computeVertexNormals();
  const helmetPaint = m.paint.clone();
  helmetPaint.onBeforeCompile = (shader) => {
    shader.vertexShader = 'varying vec3 vHelmetLocal;\n' + shader.vertexShader.replace(
      '#include <begin_vertex>', '#include <begin_vertex>\nvHelmetLocal = position - vec3(0.0,0.12,0.02);');
    shader.fragmentShader = 'varying vec3 vHelmetLocal;\n' + shader.fragmentShader.replace(
  ...
  helmetPaint.customProgramCacheKey = () => 'original-helmet-crown-v1';
  const shell = mesh(parent, shellGeometry, helmetPaint, 0, 0.12, 0.02);
  shell.scale.set(1, 1.08, 0.99);
  const visorMaterial = new T.MeshPhysicalMaterial({
    name: 'Original smoked visor', color: 0x253b45, metalness: 0.35,
    roughness: 0.13, clearcoat: 1, clearcoatRoughness: 0.07,
  });
  // SphereGeometry phi 0..PI faces +Z, the car's direction of travel.
  const visor = mesh(parent,
    new T.SphereGeometry(0.1405, 32, 12, 0.07, Math.PI - 0.14, Math.PI * 0.40, Math.PI * 0.16),
    visorMaterial, 0, 0.12, 0.021);
  visor.scale.y = 1.08;
  for (const theta of [Math.PI * 0.40, Math.PI * 0.56]) {
    const points = Array.from({ length: 25 }, (_, j) => {
      const phi = 0.07 + j / 24 * (Math.PI - 0.14);
      return new T.Vector3(-Math.cos(phi) * Math.sin(theta) * 0.141,
        0.12 + Math.cos(theta) * 0.141 * 1.08, 0.021 + Math.sin(phi) * Math.sin(theta) * 0.141);
    });
    mesh(parent, new T.TubeGeometry(new T.CatmullRomCurve3(points), 24, 0.0019, 6, false), m.dark);
  }
  for (const side of [-1, 1]) {
    const hinge = mesh(parent, new T.CylinderGeometry(0.011, 0.011, 0.006, 12), m.metal,
      side * 0.139, 0.119, 0.025);
...
export function openFrontCap(geometry: T.BufferGeometry) {
  ...
  const front = geometry.boundingBox!.max.z, next: number[] = [];
  for (let i = 0; i < index.count; i += 3) {
    const a = index.getX(i), b = index.getX(i + 1), c = index.getX(i + 2);
    if (Math.abs(position.getZ(a) - front) < 1e-6 &&
      Math.abs(position.getZ(b) - front) < 1e-6 && Math.abs(position.getZ(c) - front) < 1e-6) continue;
    next.push(a, b, c);
  }
...
export function addSidepodDuct(parent: T.Group, side: number, m: MechanicalMaterials) {
  const root = new T.Group(); root.name = 'Recessed original sidepod inlet';
  root.position.set(side * 0.53, 0, 0); root.rotation.z = side * -0.08;
  const inner = m.dark.clone(); inner.side = T.BackSide;
  const duct = new T.CylinderGeometry(1, 0.84, 0.17, 32, 1, true);
  duct.rotateX(Math.PI / 2); duct.scale(0.216, 0.065, 1);
  ...
  for (let i = -2; i <= 2; i++) {
    ...
    const fin = mesh(root, new T.BoxGeometry(extent, 0.003, 0.005), m.metal, 0, 0.025 + i * 0.017, 0.229);
    fin.name = 'Recessed cooling matrix';
  }
  const divider = mesh(root, new T.BoxGeometry(0.42, 0.008, 0.10), m.carbon, 0, 0.025, 0.353);
  divider.name = 'Inlet divider';
  mergeStatic(root); parent.add(root); return root;
}
